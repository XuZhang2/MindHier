...
def two_way_identification(all_brain_recons, all_images, model, preprocess, feature_layer=None, return_avg=True, device='cpu'):
    if len(all_images) < 2:
        raise ValueError("two_way_identification requires at least two image/reconstruction pairs.")
...
def cal_metrics(results_path, device):
    ...
    if number == 0:
        raise ValueError(f"No paired *_img.pt and *_rec.pt files found in {results_path}.")
    ...
    for im,rec in tqdm(zip(img_gray,recon_gray),total=len(all_images)):
        ssim_score.append(ssim(rec, im, channel_axis=None, gaussian_weights=True, sigma=1.5, use_sample_covariance=False, data_range=1.0))
    ...
    csv_file_path = os.path.join(results_path, f'_metrics_on_{number}samples.csv')
    df.to_csv(csv_file_path, sep=',', index=False)  # 将sep设为','
