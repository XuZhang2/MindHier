...
except ImportError:  # Support package-style imports during later refactors.
    from brainencoder.utils.constants import SUBJECT_NUM_VOXELS
    ...
    def __init__(
        ...
        if self.multi_subj:
            self.ridge = nn.ModuleList([
                torch.nn.Linear(input_size, 4096) for input_size in self.input_sizes
            ])
            ...
            for sample in x:
                ...
                if input_dim not in self.input_sizes:
                    raise ValueError(
                        ...
                        "Update SUBJECT_NUM_VOXELS if you use a custom mask."
                    )
...
class BranchDecoder(nn.Module):
    ...
    """
    def __init__(self, num_tokens, d_model=768, nhead=8, num_layers=4, dropout=0.1, activation='gelu'):
        """
        ...
        decoder_layer = TransformerDecoderLayer(
            d_model, nhead, d_model*4, dropout, activation, batch_first=False
...
class fMRI2CLIP(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            nhead_decoder (int): Number of attention heads for branch decoders.
            num_layers_decoder (int): Number of layers for branch decoders.
            dim_feedforward_decoder (int): Feedforward dimension for branch decoders.
        """
        super().__init__()

        # fMRI Encoder
        self.fMRI_encoder = FMRIEncoder(input_dim, d_model, fmri_seq_len, multi_subj)

        # Image Decoder Branch
        self.image_decoder = BranchDecoder(image_seq_len, d_model, nhead_decoder, num_layers)
        # Text Decoder Branch
        self.text_decoder = BranchDecoder(text_seq_len, d_model, nhead_decoder, num_layers)
    ...
    def forward(self, x):
        ...
        # result = [
        #     image_emb_all[i-1] - image_emb_all[i] 
        #     for i in range(1, len(image_emb_all))
        # ] + [image_emb_all[-1]]     
        ...
        return projected_image_emb, projected_text_emb, image_emb_all#, projected_text_emb
