...
try:
    from utils.constants import DEFAULT_TEST_SAMPLES, NSD_TRIALS_PER_SESSION
except ImportError:
    from brainencoder.utils.constants import DEFAULT_TEST_SAMPLES, NSD_TRIALS_PER_SESSION
...
class NSDBrainDataset(Dataset):
    def __init__(
        ...
        self.subj_list = np.arange(1,9) if multi_subject else [subj]
        if multi_subject:
            self.subj_list = self.subj_list[self.subj_list != subj]
        ...
        for s in self.subj_list:
            ...
            if is_train:
                ds = ds.shuffle(
                    ...
                    initial=2 * NSD_TRIALS_PER_SESSION,
    ...
    def __getitem__(self, idx):
        ...
        return image, voxel, image_idx

def coco_collate_fn(batch):
    """Collate function that handles both images and voxels"""
    images = torch.stack([item[0] for item in batch])
    # voxels = torch.stack([item[1] for item in batch])
    voxels = [item[1] for item in batch]
    image_idx = torch.tensor([item[2] for item in batch])
    return voxels, images, image_idx
