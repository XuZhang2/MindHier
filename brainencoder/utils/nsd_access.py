...
class NSDAccess(object):
    ...
    def __init__(self, nsd_folder, *args, **kwargs):
        ...
        # Replace through nsd_folder instead of editing source paths.
        self.coco_train = COCO(self.coco_annotation_file.format('instances', 'train2017'))
        self.coco_val = COCO(self.coco_annotation_file.format('instances', 'val2017'))
    ...
    def read_image_coco_bbox(self, image_index):
        """image_coco_bbox returns the coco bounding box of a single image or a list of images
        ...
        -------
        coco bounding box
        coco bounding box, to be used in subsequent analysis steps

            Example
            -------
            single image:
                    ci = read_image_coco_bbox(
                        [569])
            list of images:
                    ci = read_image_coco_bbox(
                        [569, 2569])
        ...
        for idx in image_index:
            ...
            coco_id = subj_info['cocoId']
            # print(str(image_index) + 'Getting bounding box for image id' + str(coco_id))
            # Locate the annotation file for this trial.
            # annot_file = self.coco_annotation_file.format(
            #     'instances', subj_info['cocoSplit'])
            # print('Getting annotations from ' + annot_file)
            # if not os.path.isfile(annot_file):
            #     print('Annotations file not found. Downloading...')
            #     self.download_coco_annotation_file()
        ...
        # Get COCO categories
            ...
            # Get bounding boxes
            if subj_info['cocoSplit'] == 'train2017':
                annotations = self.coco_train.loadAnns(self.coco_train.getAnnIds(imgIds=[coco_id]))
            else:
                annotations = self.coco_val.loadAnns(self.coco_val.getAnnIds(imgIds=[coco_id]))
            bboxes = [ann['bbox'] for ann in annotations]  # List of bounding boxes for the image
            cat_ids = [ann['category_id'] for ann in annotations]  # List of category ids for the image

            if subj_info['cocoSplit'] == 'train2017':
                cat_names = [self.coco_train.loadCats(cat_id)[0]['name'] for cat_id in cat_ids]
            else:
                cat_names = [self.coco_val.loadCats(cat_id)[0]['name'] for cat_id in cat_ids]

            # results.append((coco_cats, bboxes))
            # results.append(bboxes)


        return bboxes, cat_ids, cat_names, coco_id
    

    def read_image_size(self, image_index):
        # if not hasattr(self, 'stim_descriptions'):
        #     self.stim_descriptions = pd.read_csv(
        #         self.stimuli_description_file, index_col=0)
        ...
        for idx in image_index:
            ...
            # coco = COCO(annot_file)
            if subj_info['cocoSplit'] == 'train2017':
                h,w = self.coco_train.get_size(coco_id)
            else:
                h,w = self.coco_val.get_size(coco_id)
            # Get image size
            # annotations = coco.loadAnns(coco.getAnnIds(imgIds=[coco_id]))
            # w,h = coco.get_size(annotations)
            # bboxes = [ann['bbox'] for ann in annotations]  # List of bounding boxes for the image
        return (h,w)
