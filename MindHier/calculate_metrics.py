...
def distributed_metrics_with_csv(
    ...
    if hasattr(pipe, "fmri_vitl"):
        raise NotImplementedError(
            "distributed_metrics_with_csv is a legacy text-prompt evaluation helper. "
            "MindHierPipeline expects fMRI voxel tensors, not caption strings. "
            "Use the fMRI reconstruction/evaluation scripts for paper metrics."
        )
    pipe.hierarchy_ar.eval()
    ...
    local_images = torch.stack(local_images).to(dist.get_device())
