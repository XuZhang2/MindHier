...
def _run_git_command(command: str, default: str = "[unknown]") -> str:
    try:
        return (
...
class Args(Tap):
    run_name: str = None
    data_path: str = "data/nsd"
    images_dir: str = "data/images"
    num_sessions: int = 40
    # text_encoder_path: str = "openai/clip-vit-large-patch14"
    # text_encoder_2_path: str = "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k"
    fmri_ckpt: str = "pretrained/fmri_last.pth"
    fmri_encoder_multi_subject: bool = False
    vae_ckpt: str  = "pretrained/vqvae.safetensors"
    hierarchy_ar_ckpt: str = None
    switti_ckpt: str = None  # Deprecated alias; use hierarchy_ar_ckpt.
    exp_name: str = "mindhier_stage2"
    wandb_project: str = "mindhier-stage2"
    ...
    # architecture args
    use_swiglu_ffn = True
    use_ar = False  # False: stage-wise masked training; True: fully autoregressive masking.
    use_crop_cond = False
    ...
    # MindHier hierarchy autoregressive transformer
    tfast: int = (
        0  # torch.compile HierarchyAR; =0: off; 1: reduce-overhead; 2: max-autotune
    )
    depth: int = 30  # number of blocks in the hierarchy autoregressive transformer
    # HierarchyAR initialization
    ...
    branch: str = _run_git_command("git symbolic-ref --short HEAD || git rev-parse HEAD")
    commit_id: str = _run_git_command("git rev-parse HEAD")
    commit_msg: str = _run_git_command("git log -1").splitlines()[-1]
...
def init_dist_and_get_args():
    ...
    if args.switti_ckpt is not None and args.hierarchy_ar_ckpt is None:
        args.hierarchy_ar_ckpt = args.switti_ckpt
    
    # Dynamically update local_out_dir_path if run_name is provided
    if args.run_name is not None and args.run_name.strip() != "":
        # args.local_out_dir_path initially holds the default (e.g., ".../local_output")
        # Get the parent directory of the default local_out_dir_path
        base_dir = os.path.dirname(args.local_out_dir_path)
        # Get the default directory name (e.g., "local_output")
        original_output_dirname = os.path.basename(args.local_out_dir_path)

        # Construct the new path (e.g., ".../local_output_YOUR_RUN_NAME")
        args.local_out_dir_path = os.path.join(
            base_dir, f"{original_output_dirname}", f"{args.run_name}"
        )
    ...
    print(f"Initialized run. Output directory: {args.local_out_dir_path}")
    # set env
