...
class VectorQuantizer2(nn.Module):
    ...
    # ===================== idxBl_to_hierarchy_ar_input: teacher-forcing input for MindHier Stage 2 =====================
    def idxBl_to_hierarchy_ar_input(self, gt_ms_idx_Bl: List[torch.Tensor], return_f_hat=False) -> torch.Tensor:
        next_scales = []
        f_hats = []
        ...
        for si in range(SN - 1):
            ...
            if return_f_hat:
                f_hats.append(F.interpolate(f_hat, size=(pn_next, pn_next), mode="area")
                .view(B, C, -1)
                .transpose(1, 2)
            )
        ...
        if return_f_hat:
            return torch.cat(f_hats, dim=1)
        return torch.cat(next_scales, dim=1) if len(next_scales) else None

    # Backward-compatible alias for older scripts.
    def idxBl_to_switti_input(self, gt_ms_idx_Bl: List[torch.Tensor], return_f_hat=False) -> torch.Tensor:
        return self.idxBl_to_hierarchy_ar_input(gt_ms_idx_Bl, return_f_hat=return_f_hat)

    # ===================== get_next_autoregressive_input: used in MindHier inference for next-scale input =====================
