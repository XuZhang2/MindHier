...
import dist
from models.basic_hierarchy_ar import AdaLNBeforeHead, AdaLNSelfCrossAttn
...
class HierarchyAR(nn.Module):
    def __init__(
        ...
        # 2. text embedding
        self.pooled_embed_size = 768#768 #1280
        self.context_dim = 768#768
        self.fmri_pooler = nn.Linear(self.pooled_embed_size, self.D)
        ...
        print(
            ...
            f"    [HierarchyAR config ] embed_dim={embed_dim}, num_heads={num_heads}, "
    ...
    def forward(
        ...
        with torch.amp.autocast('cuda', enabled=False):
            pooled_prompt_embeds = self.fmri_pooler(pooled_prompt_embeds)
        ...
        num_context = len(prompt_embeds)
        # current_prompt_embeds = torch.cat(prompt_embeds[::-1], dim=1)
        current_prompt_embeds = torch.cat(prompt_embeds, dim=1)
        for block_idx, block in enumerate(self.blocks):
            # print(f'num_context{num_context}')
            # print(f'prompt_embeds_shape{current_prompt_embeds.shape}')
...
class HierarchyARHF(HierarchyAR, PyTorchModelHubMixin):
...
# Backward-compatible aliases for old checkpoints/notebooks.
Switti = HierarchyAR
