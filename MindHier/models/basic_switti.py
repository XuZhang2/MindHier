...
# Building blocks used by the MindHier hierarchy autoregressive transformer.
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.context_attn_bias = None
        
    def kv_caching(self, enable: bool):
        self.caching, self.cached_k, self.cached_v = enable, None, None
        
    def forward(self, x, context, num_context=None, freqs_cis=None):
        B, L, C = x.shape
        context_B, context_L, context_C = context.shape
        assert B == context_B
        
        def create_grouped_attention_mask(batch_size, device, group_string, context_L, num_context):
            group_sizes = list(map(int, group_string.split('_')))
            num_groups = len(group_sizes)
            
            attn_mask = torch.full((1, sum(group_size**2 for group_size in group_sizes), context_L), float('-inf'), device=device)
            q_start = 0
            for i, group_size in enumerate(group_sizes):
                q_end = q_start + group_size**2
                # print(q_start, q_end)
                segment_idx = (i * num_context) // num_groups
                kv_start = segment_idx * (context_L // num_context)
                kv_end = kv_start + (context_L // num_context)
                attn_mask[:, q_start:q_end, kv_start:kv_end] = 0
                q_start = q_end
            return attn_mask
        
        if self.context_attn_bias is None:
            self.context_attn_bias = create_grouped_attention_mask(B, x.device, "1_2_3_4_6_9_13_18_24_32", context_L, num_context)
            self.context_attn_bias = rearrange(self.context_attn_bias, "b j k -> b 1 j k")
        ...
        # if self.context_attn_bias is not None and len(self.context_attn_bias.shape)==3:
        #     self.context_attn_bias = rearrange(self.context_attn_bias, "b j k -> b 1 j k")

        dropout_p = self.attn_drop if self.training else 0.0
        # print(f'query_shape{q.shape}')
        # print(f'key_shape{k.shape}')
        # print(f'value_shape{v.shape}')
        out = (
            scaled_dot_product_attention(
                ...
                value=v,
                scale=self.scale,
                attn_mask=self.context_attn_bias[:,:,:q.shape[2],:] if self.context_attn_bias.shape[2] != q.shape[2] else self.context_attn_bias,
                dropout_p=dropout_p,
            )
            .transpose(1, 2)
            .reshape(B, L, C)
...
class AdaLNSelfCrossAttn(nn.Module):
    ...
    def forward(
        ...
        x = x + self.drop_path(self.self_attention_norm2(
            self.attn(
        ...
        if context is not None:
            x = x + self.drop_path(self.cross_attention_norm2(
            self.cross_attn(
        ...
        x = x + self.drop_path(self.ffn_norm2(
