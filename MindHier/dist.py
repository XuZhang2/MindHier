...
def initialize():
    ...
    try:
        ...
        env_world_size = 1
    has_torchrun_env = has_torchrun_env and env_world_size > 1
    has_mpi_env = any(
        key in os.environ
        for key in ("OMPI_COMM_WORLD_SIZE", "PMI_SIZE", "PMIX_RANK", "SLURM_PROCID")
    )
    is_mpirun = (not has_torchrun_env) and has_mpi_env
    # if True:
    ...
    elif not has_torchrun_env:
        os.environ["LOCAL_RANK"] = "0"
        os.environ["HOST_RANK"] = "0"
        os.environ["NUM_HOSTS"] = "1"
        os.environ["RANK"] = "0"
    ...
    if int(os.environ.get("WORLD_SIZE", "1")) == 1:
        init_file = Path(tempfile.gettempdir()) / f"mindhier_dist_{os.getpid()}"
        if init_file.exists():
            init_file.unlink()
        ...
        dist.init_process_group(backend=backend, timeout=datetime.timedelta(0, 60))
...
def allreduce(t: th.Tensor, async_op=False):
    if dist.is_initialized():
        if th.cuda.is_available() and not t.is_cuda:
            cu = t.detach().cuda()
